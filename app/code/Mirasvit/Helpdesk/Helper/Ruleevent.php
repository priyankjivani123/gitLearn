...
class Ruleevent extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        /* set attributes **/
        if ($rule->getStatusId() && (($ticket->getOrigData() === null && $this->config->getDefaultStatus() == $ticket->getStatus()->getId())
            || ($ticket->getOrigData() && $ticket->getOrigData('status_id') == $ticket->getData('status_id')))) {
            $ticket->setStatusId($rule->getStatusId());
        }
        if ($rule->getPriorityId() && ($ticket->getOrigData() === null && $this->config->getDefaultPriority() == $ticket->getPriority()->getId())
            || ($ticket->getOrigData() && $ticket->getOrigData('priority_id') == $ticket->getData('priority_id'))) {
    ...
    protected function _sendEventNotification($email, $name, $rule, $ticket)
    {
        if (!is_array($this->sentEmails) || !in_array($email, $this->sentEmails)) {
            ...
            $email = (strpos((string) $email, ',')) ? explode(',', $email) : (array) $email;
