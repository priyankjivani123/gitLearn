...
class Ticket extends \Magento\Rule\Model\Condition\AbstractCondition
{
    ...
    public function getInputType()
    {
        ...
        if (strpos((string) $attrCode, '_id') !== false || $attrCode == 'last_reply_by' || $attrCode == 'channel' ||
    ...
    public function validate(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $attrCode = $this->getAttribute();
        if (strpos((string) $attrCode, 'old_') === 0) {
        ...
        } elseif (strpos((string) $attrCode, 'hours_since_') === 0) {
        ...
        if (strpos((string) $attrCode, '_id') !== false) {
    ...
    {
        if (strpos((string) $attrCode, 'f_') === 0 || strpos($attrCode, 'old_f_') === 0) {
